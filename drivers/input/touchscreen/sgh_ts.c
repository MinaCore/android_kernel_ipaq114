...
#define IPAQ114_PENDOWN_IRQ_GPIO	89
struct continuous {
	u16 id;    /* codec id */
...
static int wm97xx_acc_pen_down(struct wm97xx *wm)
{
	...
	do {
		...
		if(status==(DATA_X|DATA_Y|DATA_P)){
			...
			input_report_abs(wm->input_dev, ABS_Y, 3900 - y);
...
static int wm97xx_acc_startup(struct wm97xx *wm)
{
	...
	dev_info(wm->dev,
		 "hpipaq114 accelerated touchscreen driver, %d samples/sec\n",
		 cinfo[sp_idx].speed);
...
static struct wm97xx_mach_ops hpipaq114_mach_ops = {
	.acc_enabled	= 1,
...
static int hpipaq114_wm97xx_probe(struct platform_device *pdev)
{
	struct wm97xx *wm = platform_get_drvdata(pdev);
	int gpio_touch_irq = IPAQ114_PENDOWN_IRQ_GPIO;
	...
	*/
	...
	wm97xx_config_gpio(wm, WM97XX_GPIO_2, WM97XX_GPIO_OUT,
			   ...
			   WM97XX_GPIO_NOWAKE);
	return wm97xx_register_mach_ops(wm, &hpipaq114_mach_ops);
}

static int hpipaq114_wm97xx_remove(struct platform_device *pdev)
{
	struct wm97xx *wm = platform_get_drvdata(pdev);
...
static struct platform_driver hpipaq114_wm97xx_driver = {
	.probe	= hpipaq114_wm97xx_probe,
	.remove	= hpipaq114_wm97xx_remove,
	.driver	= {
		.name	= "wm97xx-touch",
	},
};

static int __init hpipaq114_wm97xx_init(void)
{
	return platform_driver_register(&hpipaq114_wm97xx_driver);
}

static void __exit hpipaq114_wm97xx_exit(void)
{
	platform_driver_unregister(&hpipaq114_wm97xx_driver);
}

module_init(hpipaq114_wm97xx_init);
module_exit(hpipaq114_wm97xx_exit);
