...
static int zylonite_wm97xx_probe(struct platform_device *pdev)
{
	...
	if (cpu_is_pxa320())
		...
		gpio_touch_irq = mfp_to_gpio(MFP_PIN_GPIO23); 	// Zylonite: 26

	wm->pen_irq = IRQ_GPIO(gpio_touch_irq);
	set_irq_type(IRQ_GPIO(gpio_touch_irq), IRQ_TYPE_EDGE_BOTH);

	wm97xx_config_gpio(wm, 23, WM97XX_GPIO_IN,		// Zylonite: 15
			   WM97XX_GPIO_POL_HIGH,
			   WM97XX_GPIO_STICKY,
			   WM97XX_GPIO_WAKE);
	wm97xx_config_gpio(wm, 23, WM97XX_GPIO_OUT,		// Zylonite: 2
			   WM97XX_GPIO_POL_HIGH,
