...
#ifdef CONFIG_ANDROID_POWER
static android_suspend_lock_t pxafb_suspend_lock = {
        .name = "pxafb"
};
			     ...
			     struct fb_info *info)
{
	...
	reg = lcd_readl(fbi, LCCR0);
	lcd_writel(fbi, LCCR0, reg & ~LCCR0_BM);


	local_irq_save(flags);

	fbi->fb.var.yoffset = var->yoffset;
	fbi->fb.var.yres = var->yres;

	setup_base_frame(fbi, 1);

	lcd_writel(fbi, FBR0, fbi->fdadr[dma] | 0x3);
...
#ifdef CONFIG_ANDROID_POWER
	android_init_suspend_lock(&pxafb_suspend_lock);
#endif
